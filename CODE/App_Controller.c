...
void App_Light_Control(int num, int mode){
...
void App_Display_Update(void){
    ...
    if (lock_state==1) {
        App_Light_Control(2,1);
    }else if (lock_state==0) {
        App_Light_Control(2,0);
    }
    ...
    if (state%3==0){
        ...
        App_Light_Control(1,1);
        App_Light_Control(4,0);
        App_Light_Control(5,0);
        App_Light_Control(6,0);
        App_Light_Control(7,0);
        App_Light_Control(8,0);
...
void App_Key_Scan(void){
    ...
    if (bstate[0]==0&&bstate_last[0]==1){
        ...
        select_state++;
    }else if (bstate[1]==0&&bstate_last[1]==1&&state%3==0){   //调整
        TIM2->CNT=0;
    }else if (bstate[1]==1&&bstate_last[1]==0&&state%3==0){   //确认
        if (TIM2->CNT>20000){
            hour=0;
            min=0;
            sec=0;
